...
namespace Workfolio\Settings\Manager\Themes;
		...
		if ( $this->installed() ) {
			...
			if ( ! $this->active() && current_user_can( 'switch_themes' ) ) {
				...
				$actions[] = sprintf(
					...
					esc_attr( sprintf( __( 'Activate %s', 'backdrop' ), $this->label() ) ),
					esc_html__( 'Activate', 'backdrop' )
				);
			}
			...
			if ( ! $this->active() && current_user_can( 'customize' ) ) {
				...
				$actions[] = sprintf(
					'<a class="button button-primary load-customize hide-if-no-customize" href="%s">%s</a>',
					esc_url( $this->customizeUrl() ),
					esc_html__( 'Live Preview', 'backdrop' )
				);
			}
